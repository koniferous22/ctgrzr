...
class CtgrzrTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(self.operation_file, "w") as f:
            f.write(
                yaml.dump(
                    {
                        self.category1: 'echo "Category1 - {}" >> '
                        + f'"{self.side_effect_file}"',
                        self.category2: 'echo "Category2 - {}" >> '
                        + f'"{self.side_effect_file}"',
                    }
                )
            )
    ...
    def test_add_remove(self):
        args_add_path1 = self.arg_parser.parse_args(
            ["add", str(self.file1), self.category1]
        )
        args_add_path2 = self.arg_parser.parse_args(
            ["add", str(self.file2), self.category1, self.category2]
        )
        args_add_path3 = self.arg_parser.parse_args(
            ["add", str(self.file3), self.category2]
        )
        self.assertEqual(cli(args_add_path1), 0)
        self.assertEqual(cli(args_add_path2), 0)
        self.assertEqual(cli(args_add_path3), 0)

        args_add_symlink_without_option = self.arg_parser.parse_args(
            ["add", str(self.file1_symlink), self.category1]
        )
        with self.assertRaises(Exception):
            cli(args_add_symlink_without_option)

        args_add_not_exist = self.arg_parser.parse_args(
            ["add", str(self.not_exists), self.category1]
        )
        with self.assertRaises(Exception):
            cli(args_add_not_exist)

        args_remove_path1 = self.arg_parser.parse_args(
            ["remove", str(self.file1), self.category1]
        )
        args_remove_path2 = self.arg_parser.parse_args(
            ["remove", str(self.file2), self.category2]
        )
        self.assertEqual(cli(args_remove_path1), 0)
        self.assertEqual(cli(args_remove_path2), 0)
        args_remove_not_exist = self.arg_parser.parse_args(
            ["remove", str(self.not_exists), self.category1]
        )
        with self.assertRaises(Exception):
            cli(args_remove_not_exist)

        args_add_symlink = self.arg_parser.parse_args(
            ["add", "-s", str(self.file1_symlink), self.category1]
        )
    ...
    def test_autoadd(self):
        ...
        args_autoadd_symlink_error = self.arg_parser.parse_args(
            ["autoadd", str(self.template_config_path)]
        )
        with self.assertRaises(Exception):
            cli(args_autoadd_symlink_error)
        run_command(f"rm {self.file3}", check=True)
        args_autoadd_with_symlink = self.arg_parser.parse_args(
            ["autoadd", "-s", str(self.template_config_path)]
        )
        ...
        args_autoadd = self.arg_parser.parse_args(
            ["autoadd", str(self.template_config_path)]
        )
        ...
        self.assertDictEqual(
            config,
            {self.category1: [self.file1, self.file2], self.category2: [self.file2]},
        )
    ...
    def test_search_symlinks(self):
        ...
        actual_symlinks = set(
            [symlink for symlink in search_symlinks_in_directory(self.root_path)]
        )
